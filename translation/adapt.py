...
def test_image_folder(cfg, ae, ebm, run_dir, iteration=0, device='cuda', n_sample=3):
	...
	for i, file in enumerate(source_files):
		...
		if is_image_file(file):
			...
			latents = torch.cat((latent.unsqueeze(0), latent_q), dim=0)
			...
			print('img_pair_shape: ',image_pair.shape)
			total_num_img = image_pair.shape[0]
...
def train(cfg, logger):
	...
	data_root = os.path.join(cfg.DATA.ROOT, cfg.DATA.NAME)
	print(data_root)
	source_dataset = ImageFolder(os.path.join(os.path.join(data_root, 'val'),cfg.DATA.SOURCE), transform=transform)
	...
	target_dataset = ImageFolder(os.path.join(data_root, 'val/' + cfg.DATA.TARGET), transform=transform)
	...
	for epoch in trange(0,50,desc='epoch'):
		iterations += 1
		...
		if iterations % 100 == 0:
			...
			with torch.no_grad():
				...
				out = torch.cat((source_img[:nrow], out), dim=0) # source_original, source_reconstructed, source_to_target_transformed
