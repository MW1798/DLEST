...
from torchmetrics.image.lpip import LearnedPerceptualImagePatchSimilarity as ploss


from augment import DiffAugment
# policy = 'color,translation,cutout'
policy = ''
perceptual_loss_criteria = ploss(net_type='vgg').cuda()
...
class Model(nn.Module):
	...
	def forward(self, x, lod, blend_factor, d_train, ae,pix=False):
		if lod < -1:
			current_policy = ''
		else:
		...
		if ae:
			...
			# pixel loss
			x_encode , _ = self.encode(x, lod, blend_factor)
			x_encode = x_encode.squeeze()
			rec = self.generate(lod, blend_factor,count=x.shape[0], z=x_encode, mixing=False, noise=True)
			# x_encode.detach()
			# del x_encode
			
			SSIM_loss = ssim_loss(x,rec,window_size=5,reduction='mean',max_val=1.0).detach()
   			# # L1 Pix loss
			# pixel_loss_criteria = torch.nn.L1Loss()
			# pixel_loss = pixel_loss_criteria(x,rec)
   
			# Perceptual Loss
			x = x.repeat(1,3,1,1)
			rec = rec.repeat(1,3,1,1)
			perceptual_loss = perceptual_loss_criteria(x,rec)	

			# x.detach()
			# rec.detach()
			# del x,rec
			# torch.cuda.empty_cache()
			return perceptual_loss,SSIM_loss
