...
class LODDriver:
    def __init__(self, cfg, logger, world_size, dataset_size):
        ...
        if world_size == 1:
            self.lod_2_batch = cfg.TRAIN.LOD_2_BATCH_1GPU # read from configuration LOD_2_BATCH_1GPU
    ...
    def get_batch_size(self):
        return self.lod_2_batch[min(self.lod, len(self.lod_2_batch) - 1)] # read batch size from configuration file LOD_2_BATCH_1GPU
    ...
    def set_epoch(self, epoch, optimizers):
        ...
        if self.cfg.TRAIN.EPOCHS_PER_LOD == 0: ########## TRAIN.EPOCHS_PER_LOD is initialized to 30 in the configuration file
            self.lod = self.cfg.MODEL.LAYER_COUNT - 1
            return

        new_lod = min(self.cfg.MODEL.LAYER_COUNT - 1, epoch // self.cfg.TRAIN.EPOCHS_PER_LOD) #### ?? change lod every 30 (EPOCHS_PER_LOD) epochs,
                                                                                                ### first time run this will change lod from -1 to 0
        if new_lod != self.lod:
            self.lod = new_lod
            self.logger.info("#" * 80)
