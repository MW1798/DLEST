...
class TFRecordsDataset:
	def __init__(self, cfg, logger, rank=0, world_size=1, buffer_size_mb=200, channels=3, seed=None, train=True, needs_labels=False):
		...
		data_root = Path(cfg.DATA.ROOT) # point to data root
		dataset_root = data_root / cfg.DATASET.ROOT # point to train/val subdir
	...
	def data_sampler(self, dataset, shuffle, distributed):
		...
		if shuffle:
			return data.RandomSampler(dataset,generator=torch.Generator(device='cuda'))
	...
	def loader(self, dataset, batch_size, image_size=4):
		...
		loader = DataLoader(dataset, batch_size=batch_size, num_workers=0, drop_last=True,pin_memory=True, sampler=sampler,generator=torch.Generator(device='cuda'))
		return loader

	def reset(self, lod, batch_size):
		self.batch_size = batch_size

		img_size = 2 ** lod # initialize: 2 ** MAX_RESOLUTION_LEVEL= 2 ** 8 = 256  first epoch: 2**2=4
